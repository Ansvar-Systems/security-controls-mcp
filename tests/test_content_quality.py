...
class TestControlContentQuality:
    ...
    def test_all_controls_have_nonempty_names(self, scf_data):
        ...
        for control in scf_data.controls:
            ...
            assert name and len(name.strip()) > 0, \
                f"Control {control.get('id')} has empty name"
    ...
    def test_control_descriptions_minimum_length(self, scf_data):
        ...
        assert len(short_descriptions) == 0, \
    ...
    def test_no_placeholder_text_in_descriptions(self, scf_data):
        ...
        assert len(issues) == 0, \
            f"Controls with placeholder text: {issues[:10]}"
    ...
    def test_control_ids_follow_pattern(self, scf_data):
        ...
        assert len(invalid_ids) == 0, \
            f"Controls with invalid ID pattern: {invalid_ids[:10]}"
    ...
    def test_control_weights_are_valid(self, scf_data):
        ...
        assert len(invalid_weights) == 0, \
            f"Controls with invalid weights: {invalid_weights[:10]}"
    ...
    def test_control_domains_are_nonempty(self, scf_data):
        ...
        assert len(empty_domains) == 0, \
            f"Controls with empty domains: {empty_domains[:10]}"
...
class TestFrameworkMappingQuality:
    ...
    def test_framework_mapping_ids_are_nonempty(self, scf_data):
        ...
        assert len(issues) == 0, \
            f"Empty framework mapping IDs found: {issues[:10]}"
...
class TestSearchQuality:
    ...
    def test_common_security_terms_return_results(self, scf_data):
        ...
        for term in common_terms:
            ...
            assert len(results) > 0, \
                f"No results for common security term: {term}"
    ...
    def test_search_results_contain_query_term(self, scf_data):
        ...
        for result in results:
            ...
            assert query in name or query in desc or query in snippet, \
    ...
    def test_search_relevance_scoring(self, scf_data):
        ...
        for result in results:
            ...
            assert relevance > 0, \
                f"Result {result.get('control_id')} has non-positive relevance"
...
class TestFrameworkSpecificContent:
    ...
    def test_dora_has_ict_controls(self, scf_data):
        ...
        assert "ict" in descriptions or "operational" in descriptions or "resilience" in descriptions, \
    ...
    def test_pci_dss_has_payment_controls(self, scf_data):
        ...
        assert "card" in descriptions or "payment" in descriptions or "transaction" in descriptions or "data" in descriptions, \
    ...
    def test_hipaa_has_health_controls(self, scf_data):
        ...
        assert "health" in descriptions or "patient" in descriptions or "medical" in descriptions or "phi" in descriptions or "protected" in descriptions, \
    ...
    def test_gdpr_has_privacy_controls(self, scf_data):
        ...
        assert "privacy" in descriptions or "personal" in descriptions or "data protection" in descriptions or "consent" in descriptions, \
...
class TestDataConsistency:
    ...
    def test_no_duplicate_control_ids(self, scf_data):
        ...
        assert len(set(duplicates)) == 0, \
            f"Duplicate control IDs found: {set(duplicates)}"
    ...
    def test_all_mapped_frameworks_exist_in_index(self, scf_data):
        ...
        assert len(missing_frameworks) == 0, \
